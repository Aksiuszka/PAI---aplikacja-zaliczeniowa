...
namespace info_2022.Data
{
    public class InfoSeeder
    {
        ...
        private static void SeedPytankos(ApplicationDbContext dbContext)
        {
            if (!dbContext.Pytankos.Any())
            {
                var pytanie = new Pytanko()
                {
                    Imie = "Joanna",
                    Adres = "jarosinska@poczta.pl",
                    Tresc = "W jaki sposób można zostać autorem tekstów?Mamkilka pomysłów na tutoriale dla studentów.",
                    Odpowiedz = false
                };

                dbContext.Set<Pytanko>().Add(pytanie);
